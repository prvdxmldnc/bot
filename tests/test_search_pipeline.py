...
def test_synonym_retry_finds_spunbond(monkeypatch):
    ...
    assert payload["decision"]["query_retry"].startswith("спанбонд")
    assert any(query.startswith("спанбонд") for query in calls)
...
def test_no_candidates_clarification_has_pagination(monkeypatch):
    async def fake_search(_session, query, limit=5, category_ids=None, product_ids=None):
        if query == "спанбонд":
            return [
                {"id": i, "title_ru": f"Спанбонд {'белый' if i % 2 == 0 else 'черный'} {i}мм", "sku": f"SB-{i}"}
                for i in range(1, 58)
            ]
        return []
    ...
    monkeypatch.setattr(
        search_pipeline,
        "build_facet_options",
        lambda candidates, max_values=30: (
            "цвет",
            [
                {"id": f"opt_{i}", "label": f"Цвет {i}", "apply": {"append_tokens": [f"цвет{i}"]}}
                for i in range(1, 58)
            ],
        ),
    )
...
def test_clarification_pagination_offset_returns_next_page(monkeypatch):
    ...
    assert len(clarification["options"]) == 10
    assert clarification["options"][0]["label"] == "Цвет 11"
...
def test_synonym_retry_sinteponn_to_sintepon(monkeypatch):
    calls = []

    async def fake_search(_session, query, limit=5, category_ids=None, product_ids=None):
        calls.append(query)
        if query == "синтепон":
            return [{"id": 88, "title_ru": "Синтепон 60", "sku": "SIN-60"}]
        return []

    monkeypatch.setattr(search_pipeline, "search_products", fake_search)
    monkeypatch.setattr(search_pipeline, "get_alias_map", lambda *args, **kwargs: asyncio.sleep(0, result={"синтепонн": "синтепон"}))
    monkeypatch.setattr(search_pipeline, "find_org_alias_candidates", lambda *args, **kwargs: asyncio.sleep(0, result=[]))
    monkeypatch.setattr(search_pipeline, "search_history_products", lambda *args, **kwargs: asyncio.sleep(0, result=[]))
    monkeypatch.setattr(search_pipeline, "count_org_candidates", _count_zero)
    monkeypatch.setattr(search_pipeline, "parse_order_text", lambda q: [{"query": q, "raw": q, "query_core": q}])
    ...
    payload = asyncio.run(search_pipeline.run_search_pipeline(DummySession(), org_id=42, user_id=None, text="синтепонн"))

    assert payload["results"]
    assert "синтепон" in calls


def test_synonym_trace_contains_retry_fields(monkeypatch):
    async def fake_search(*args, **kwargs):
        return []

    monkeypatch.setattr(search_pipeline, "search_products", fake_search)
    monkeypatch.setattr(search_pipeline, "get_alias_map", lambda *args, **kwargs: asyncio.sleep(0, result={"спандбонд": "спанбонд"}))
    ...
    payload = asyncio.run(search_pipeline.run_search_pipeline(DummySession(), org_id=42, user_id=None, text="спандбонд 70"))

    assert "synonym_retry_attempted" in payload["decision"]
